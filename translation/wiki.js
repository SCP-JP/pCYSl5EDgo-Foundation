...
WIKIDOT.render.fixAvatarHover.showHover = function (k) {
    ...
    if (j.height == 0) {
        ...
        c = 48;
        b = true;
        YAHOO.util.Event.addListener(j, "load", function (s, d) {
            var e = this;
            YAHOO.util.Dom.setXY(d, [g - (e.width / 2 + 8), f - (e.height / 2 + 8)])
        }, l)
    } else {
        o = j.height;
        c = j.width
    ...
    editClick: function (g) {
        ...
        if (WIKIDOT.page.vars.editSectionsActive == false) {
            ...
            for (var h = 0;
                ...
                YAHOO.util.Event.addListener(o, "click", WIKIDOT.page.listeners.editSection);
                $j(o).hide();
...
WIKIDOT.page.utils = {
    scrollToReference: function (b) { OZONE.visuals.scrollTo(b, { blink: true }) }, addCloseToActionArea: function () {
        ...
        if (!b || b == "recheck") {
            ...
            case "wantedpages": WIKIDOT.modules.SiteToolsModule.listeners.wantedPages();
                break;
            case "orphanedpages": WIKIDOT.modules.SiteToolsModule.listeners.orphanedPages();
                break;
            case "draftpages": WIKIDOT.modules.SiteToolsModule.listeners.draftPages();
                break;
            case "editpage": WIKIDOT.page.listeners.editClick();
                break;
            case "edittags": WIKIDOT.page.listeners.editTags();
                break;
            case "history": WIKIDOT.page.listeners.historyClick();
                break;
            case "files": WIKIDOT.page.listeners.filesClick();
                break
        }
    }
};
